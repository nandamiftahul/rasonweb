...
import numpy as np
import sqlite3
from flask import Blueprint, jsonify, request, session, redirect, url_for, Response, render_template, send_file
...
@api.route("/api/user_expiry", methods=["PUT"])
@login_required
...
@api.route("/api/user_pages", methods=["PUT"])
@login_required
...
@api.route("/api/filter", methods=["POST"])
@login_required
def api_filter():
    """
...
@api.route("/api/filemeta/<site>/<filename>")
@login_required
...
@api.route("/api/time_accuracy/<site>")
@login_required
...
@api.route("/api/data_availability")
@login_required
...
@api.route("/api/bufrmap_full", methods=["GET","POST","PUT","DELETE"])
@login_required
...
@api.route("/api/bufrmap/<site>", methods=["GET", "POST", "PUT", "DELETE"])
@login_required
...
@api.route("/api/serial_lookup/<serial>")
@login_required
...
@api.route("/api/insert_from_ftp/<site>/<filename>")
@login_required
...
@api.route("/api/redownload/<site>/<filename>")
@login_required
...
@api.route("/download/<site>/<filename>")
@login_required
...
@api.route("/load_from_ftp/<site>/<filename>")
@login_required
...
@api.route("/all_levels")
@login_required
def all_levels_route():
    store = get_user_store()
    return jsonify(store["levels"])

@api.route("/download_wmo/<site>/<filename>")
@login_required
...
@api.route("/upload_bufr", methods=["POST"])
@login_required
def upload_bufr():
    """
